...
DEFAULT_OPTIONS = {'width': '800', 'height': '200', 'color': 'ff9900',
                   'expected': '0', 'bgcolor': 'ffffff00', 'spent':'true',
                   'colorspent': '40af30', 'wecolor':'ccccccaa', 'colorexpected': 'ffddaa',
...
class BurndownChart(EstimationToolsBase, WikiMacroBase):
     ...
     * `colorspent`: color for spent hours graph specified as 6-letter string of hexadecimal values in the format `RRGGBB`.
       Defaults to ffddaa, a nice yellow.
    ...
    def _calculate_timetable_spent(self, options, query_args, req):
        ...
        for t in tickets:
            ...
            history_cursor.execute("SELECT "
                ...
                "FROM ticket t, ticket_change c "
                "WHERE t.id = %s and c.ticket = t.id and (c.field=%s or c.field='status')"
                "ORDER BY c.time ASC", [t['id'], self.spent_field])
